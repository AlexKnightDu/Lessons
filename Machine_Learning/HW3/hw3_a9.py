...
def train(kernel):
    ...
    fout = open('./result/M' + kernel_type[kernel] + '_' + time_stamp + '.out', 'w+')
    ...
    # Cost
    c_para = list(np.logspace(5,10,6,base=4)/1e4)
    g_para = [1.0/123, 0.02, 0.03 ]
            ...
            for r in r_para:
                if (str(kernel) == '1'):
                    for d in d_para:
                        ...
                        fout.write(param + ' ')
                        param = svm_parameter(param + ' -b 1 -m 10000 -h 1 -q')
